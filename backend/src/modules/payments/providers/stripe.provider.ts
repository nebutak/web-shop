...
export const stripeProvider: PaymentProviderAdapter = {
  ...
  async createPayment(): Promise<CreatePaymentResult> {
    throw new AppError("Stripe provider is not configured yet", HTTP_STATUS.BAD_REQUEST);
  },

  async verifyCallback(): Promise<VerifyPaymentResult> {
    throw new AppError("Stripe callback verification is not configured yet", HTTP_STATUS.BAD_REQUEST);
  },
};
